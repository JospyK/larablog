    ...
    <div class="row">
      ...
      <div class="col-md-4">
        <div class="card" style="margin-top: 30px; padding: 20px;">
            ...
            <dl class="dl-horizontal fit">
              <label>Author : </label>
              <a href="{{route('blog.users', $post->user->first_name.'.'.$post->user->last_name)}}">{{ $post -> user -> first_name }} {{ $post -> user -> first_name }}</a>
            </dl>
      ...
      <!--Card-->
        <div class="card card-personal" style="margin-top: 30px; padding: 20px;">
            ...
            <div class="card-block">
                ...
                <hr>
                <a class="card-meta" href="{{route('blog.users', $post->user->first_name.'.'.$post->user->last_name)}}"><span><i class="fa fa-file"></i> {{$post->user->posts->where('statut', '=','success')->count()}} {{ ($post->user->posts->count()<=1)? " Post": " Posts"}} </span></a>
            </div>
            <!--/.Card content-->
        </div>
      <!--/.Card-->
